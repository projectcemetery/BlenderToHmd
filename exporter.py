...
class haxe_io_Input:
    ...
    _hx_methods = ["readByte", "set_bigEndian", "readFloat", "readInt32"]
...
class io_Animation:
    _hx_class_name = "io.Animation"
    __slots__ = ("objectAnimations", "frames")
    _hx_fields = ["objectAnimations", "frames"]
    _hx_methods = ["addObjectAnimation"]

    def __init__(self,frames):
        self.frames = frames
        self.objectAnimations = list()

    def addObjectAnimation(self,anim):
        self.objectAnimations.append(anim)

io_Animation._hx_class = io_Animation
...
class io_Exporter:
    ...
    _hx_methods = ["int32ToFloat", "getSkin", "writeVertex", "addModel", "addAnimation", "write"]

    def __init__(self):
        self.dataBytes = None

    def int32ToFloat(self,v):
        tmp = haxe_io_Bytes.alloc(4)
        tmp.b[0] = ((v & 255) & 255)
        tmp.b[1] = (((v >> 8) & 255) & 255)
        tmp.b[2] = (((v >> 16) & 255) & 255)
        tmp.b[3] = (HxOverrides.rshift(v, 24) & 255)
    ...
    def getSkin(self,skin):
        ...
        while (_g < len(_g1)):
            ...
            njoint.transpos = hxd_fmt_hmd_Position()
            njoint.transpos.x = 0
            njoint.transpos.y = 0
            njoint.transpos.z = 0
    ...
    def writeVertex(self,geom,_hx_bytes):
        _g = 0
        _g1 = geom.vertexArray
        while (_g < len(_g1)):
            ...
            if geom.hasWeights:
                ...
                _g3 = vert.weights
                while (_g2 < len(_g3)):
                    w = (_g3[_g2] if _g2 >= 0 and _g2 < len(_g3) else None)
                    _g2 = (_g2 + 1)
                    _hx_bytes.writeFloat(w.weight)
                idx = 0
                _g21 = 0
                _g31 = vert.weights
                while (_g21 < len(_g31)):
                    w1 = (_g31[_g21] if _g21 >= 0 and _g21 < len(_g31) else None)
                    _g21 = (_g21 + 1)
                    idx = ((idx << 8) | w1.boneIndex)
                _hx_bytes.writeFloat(self.int32ToFloat(idx))
    ...
    def addModel(self,hmd,model):
        ...
        if egeom.hasWeights:
            ngeom.vertexStride = (ngeom.vertexStride + 4)
            pw = hxd_fmt_hmd_GeometryFormat("weights",python_internal_ArrayImpl._get([1, 2, 3, 4], (model.skin.bonePerVertex - 1)))
            ngeom.vertexFormat.append(pw)
            pw = hxd_fmt_hmd_GeometryFormat("indexes",9)
            ngeom.vertexFormat.append(pw)
        ...
        self.writeVertex(egeom,_hx_bytes)
        ngeom.indexPosition = len(_hx_bytes.b.b)
        _g = 0
        _g1 = egeom.indexArray
        while (_g < len(_g1)):
    ...
    def addAnimation(self,hmd,anim):
        _hx_bytes = self.dataBytes
        nanim = hxd_fmt_hmd_Animation()
        nanim.name = "Animation"
        nanim.frames = anim.frames
        nanim.loop = True
        nanim.speed = 1
        nanim.sampling = 30
        nanim.dataPosition = len(_hx_bytes.b.b)
        nanim.objects = list()
        _g = 0
        _g1 = anim.objectAnimations
        while (_g < len(_g1)):
            ao = (_g1[_g] if _g >= 0 and _g < len(_g1) else None)
            _g = (_g + 1)
            nanimObj = hxd_fmt_hmd_AnimationObject()
            nanimObj.name = ao.name
            nanimObj.flags = 0
            def _hx_local_2():
                nanimObj.flags = (nanimObj.flags | ((1 << hxd_fmt_hmd_AnimationFlag.HasPosition.index)))
                return nanimObj.flags
            nanimObj.flags = ((_hx_local_2()) | ((1 << hxd_fmt_hmd_AnimationFlag.HasRotation.index)))
            nanim.objects.append(nanimObj)
            _g2 = 0
            _g3 = ao.data
            while (_g2 < len(_g3)):
                dat = (_g3[_g2] if _g2 >= 0 and _g2 < len(_g3) else None)
                _g2 = (_g2 + 1)
                _hx_bytes.writeFloat(dat.x)
                _hx_bytes.writeFloat(dat.y)
                _hx_bytes.writeFloat(dat.z)
                if ao.hasRotation:
                    _hx_bytes.writeFloat(dat.qx)
                    _hx_bytes.writeFloat(dat.qy)
                    _hx_bytes.writeFloat(dat.qz)
        hmd.animations.append(nanim)
    ...
    def write(self,filepath,scene):
        ...
        _g = 0
        _g1 = scene.modelArray
        while (_g < len(_g1)):
            m = (_g1[_g] if _g >= 0 and _g < len(_g1) else None)
            _g = (_g + 1)
            self.addModel(hmd,m)
        _g2 = 0
        _g11 = scene.animations
        while (_g2 < len(_g11)):
            a = (_g11[_g2] if _g2 >= 0 and _g2 < len(_g11) else None)
            _g2 = (_g2 + 1)
            self.addAnimation(hmd,a)
...
class io_Model:
    ...
    __slots__ = ("name", "parent", "geometry", "skin")
    _hx_fields = ["name", "parent", "geometry", "skin"]
...
class io_ObjectAnimation:
    _hx_class_name = "io.ObjectAnimation"
    __slots__ = ("name", "hasRotation", "hasScale", "data")
    _hx_fields = ["name", "hasRotation", "hasScale", "data"]
    _hx_methods = ["addFrame"]

    def __init__(self,hasRotation,hasScale):
        self.name = None
        self.data = list()
        self.hasRotation = hasRotation
        self.hasScale = hasScale

    def addFrame(self,x,y,z,qx = None,qy = None,qz = None,sx = None,sy = None,sz = None):
        pos = hxd_fmt_hmd_Position()
        pos.x = x
        pos.y = y
        pos.z = z
        pos.qx = 0
        ...
        pos.sy = 1
        pos.sz = 1
        if (((self.hasRotation and ((qx is not None))) and ((qy is not None))) and ((qz is not None))):
            pos.qx = qx
            pos.qy = qy
            pos.qz = qz
        if (((self.hasScale and ((sx is not None))) and ((sy is not None))) and ((sz is not None))):
            pos.sx = sx
            pos.sy = sy
            pos.sz = sz
        self.data.append(pos)

io_ObjectAnimation._hx_class = io_ObjectAnimation


class io_Scene:
    _hx_class_name = "io.Scene"
    __slots__ = ("modelArray", "animations")
    _hx_fields = ["modelArray", "animations"]
    _hx_methods = ["addModel", "addAnimation"]
